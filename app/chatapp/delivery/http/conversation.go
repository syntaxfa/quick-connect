package http

import (
	"net/http"

	"github.com/labstack/echo/v4"
	"github.com/syntaxfa/quick-connect/app/chatapp/service"
	"github.com/syntaxfa/quick-connect/pkg/auth"
	"github.com/syntaxfa/quick-connect/pkg/servermsg"
	"github.com/syntaxfa/quick-connect/types"
)

// GetActiveConversation docs
// @Summary GetActiveConversation
// @Description get user active conversation
// @Tags Chat
// @Accept json
// @Produce json
// @Success 200 {object} service.Conversation
// @Failure 500 {string} something went wrong
// @Security JWT
// @Router /conversations/active [GET].
func (h Handler) GetActiveConversation(c echo.Context) error {
	claims, cErr := auth.GetUserClaimFormContext(c)
	if cErr != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, cErr.Error())
	}

	resp, sErr := h.svc.GetUserActiveConversation(c.Request().Context(), claims.UserID)
	if sErr != nil {
		return servermsg.HTTPMsg(c, sErr, h.t)
	}

	return c.JSON(http.StatusOK, resp)
}

// ConversationNewList docs
// @Summary ConversationNewList
// @Description new conversation list
// @Tags Chat
// @Accept json
// @Produce json
// @Param Request body service.ListConversationsRequest true "check token validation"
// @Success 200 {object} service.ListConversationsResponse
// @Failure 400 {string} string Bad Request
// @Failure 422 {object} servermsg.ErrorResponse
// @Failure 500 {string} something went wrong
// @Security JWT
// @Router /conversations/new [POST].
func (h Handler) ConversationNewList(c echo.Context) error {
	var req service.ListConversationsRequest
	if bErr := c.Bind(&req); bErr != nil {
		return echo.NewHTTPError(http.StatusBadRequest)
	}

	req.Statuses = []service.ConversationStatus{service.ConversationStatusNew}

	resp, sErr := h.svc.ListConversations(c.Request().Context(), req)
	if sErr != nil {
		return servermsg.HTTPMsg(c, sErr, h.t)
	}

	return c.JSON(http.StatusOK, resp)
}

// OwnConversationList docs
// @Summary OwnConversationList
// @Description your conversation list
// @Tags Chat
// @Accept json
// @Produce json
// @Param Request body service.ListConversationsRequest true "check token validation"
// @Success 200 {object} service.ListConversationsResponse
// @Failure 400 {string} string Bad Request
// @Failure 422 {object} servermsg.ErrorResponse
// @Failure 500 {string} something went wrong
// @Security JWT
// @Router /conversations [POST].
func (h Handler) OwnConversationList(c echo.Context) error {
	userClaims, gErr := auth.GetUserClaimFormContext(c)
	if gErr != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, gErr.Error())
	}

	var req service.ListConversationsRequest
	if bErr := c.Bind(&req); bErr != nil {
		return echo.NewHTTPError(http.StatusBadRequest)
	}

	req.AssignedSupportID = userClaims.UserID

	resp, sErr := h.svc.ListConversations(c.Request().Context(), req)
	if sErr != nil {
		return servermsg.HTTPMsg(c, sErr, h.t)
	}

	return c.JSON(http.StatusOK, resp)
}

// OpenConversation docs
// @Security JWT
// @Router /conversations/{conversationID}/open [GET].
// @Summary OpenConversation
// @Description open conversation for chatting
// @Tags Chat
// @Accept json
// @Produce json
// @Param conversationID path string true "conversation ID"
// @Failure 404 {string} conversation does not exist
// @Failure 500 {string} something went wrong.
func (h Handler) OpenConversation(c echo.Context) error {
	claims, gErr := auth.GetUserClaimFormContext(c)
	if gErr != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, gErr.Error())
	}

	resp, sErr := h.svc.OpenConversation(c.Request().Context(), types.ID(c.Param("conversationID")), claims.UserID)
	if sErr != nil {
		return servermsg.HTTPMsg(c, sErr, h.t)
	}

	return c.JSON(http.StatusOK, resp)
}

// CloseConversation docs
// @Security JWT
// @Router /conversations/{conversationID}/close [GET].
// @Summary CloseConversation
// @Description close conversation for chatting
// @Tags Chat
// @Accept json
// @Produce json
// @Param conversationID path string true "conversation ID"
// @Failure 404 {string} conversation does not exist
// @Failure 409 {string} this conversation already closed
// @Failure 500 {string} something went wrong.
func (h Handler) CloseConversation(c echo.Context) error {
	claims, gErr := auth.GetUserClaimFormContext(c)
	if gErr != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, gErr.Error())
	}

	resp, sErr := h.svc.CloseConversation(c.Request().Context(), types.ID(c.Param("conversationID")), claims.UserID)
	if sErr != nil {
		return servermsg.HTTPMsg(c, sErr, h.t)
	}

	return c.JSON(http.StatusOK, resp)
}

// ConversationDetail docs
// @Security JWT
// @Router /conversations/{conversationID} [GET].
// @Summary ConversationDetail
// @Description close conversation for chatting
// @Tags Chat
// @Accept json
// @Produce json
// @Param conversationID path string true "conversation ID"
// @Failure 404 {string} conversation does not exist
// @Failure 500 {string} something went wrong.
func (h Handler) ConversationDetail(c echo.Context) error {
	claims, gErr := auth.GetUserClaimFormContext(c)
	if gErr != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, gErr.Error())
	}

	resp, sErr := h.svc.GetConversationByID(c.Request().Context(), types.ID(c.Param("conversationID")), claims.UserID, claims.Roles)
	if sErr != nil {
		return servermsg.HTTPMsg(c, sErr, h.t)
	}

	return c.JSON(http.StatusOK, resp)
}
