...
const queryIsConversationExistByID = `SELECT EXISTS (
	SELECT 1
	FROM conversations
	WHERE id = $1
);`

func (d *DB) IsConversationExistByID(ctx context.Context, conversationID types.ID) (bool, error) {
	const op = "repository.postgres.exist.IsConversationExistByID"

	var exists bool
	if qErr := d.conn.Conn().QueryRow(ctx, queryIsConversationExistByID, conversationID).Scan(&exists); qErr != nil {
		if errors.Is(qErr, pgx.ErrNoRows) {
			return false, nil
		}
